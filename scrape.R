...
setwd("C:/Users/Mike/Documents/R/cdjapan/")
...
# 三森すずこ、南條愛乃、園田海未、絢瀬絵里、ラブライブ ぬいぐるみ, 音のレガート, SEIKO 時計
# R doesn't like it when you give it the UTF-8 encoded versions of the name even though it works
# fine in the URL.
# Update 3/16/23. importing stringi and setting `stri_enc_set("Shift_JIS")` will probably allow it to work with proper encoding.
# Update 5/3/23. Above solution did not work, but I'm convinced it's possible.
...
Sys.sleep(15) #Give computer a little more time to establish internet connection.

for(i in 1L:length(search_term)) {
  if(i > 1L) {
    Sys.sleep(4L) 
  }
  base_url <- str_c("https://www.cdjapan.co.jp/searchuni?q=", search_term[i], "&order=newdesc")
...
# Can probably rewrite this with an apply.
for(j in 1L:length(newest_urls)) {
